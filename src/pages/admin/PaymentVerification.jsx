...
const PaymentVerification = () => {
  ...
  const handleProcessPayment = async (paymentId, action, rejectionReason = null) => {
    try {
      ...
      // Use the correct endpoint that creates earnings records
      let res;
      if (action === 'approve') {
        res = await axios.post(`/api/section-payments/${paymentId}/approve`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else if (action === 'reject') {
        res = await axios.post(`/api/section-payments/${paymentId}/reject`, {
          rejectionReason
        }, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleApproveAll = async () => {
    if (!window.confirm(`Are you sure you want to approve all ${payments.length} pending payments?`)) {
      return;
    }
    ...
    try {
      ...
      await Promise.all(
        payments.map(payment => 
          axios.post(`/api/section-payments/${payment._id}/approve`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          })
        )
      );
      ...
      toast.error('Failed to approve all payments');
    }
  };

  const handleRejectAll = async () => {
    const reason = prompt('Enter rejection reason for all payments:');
    if (reason === null) return;

    if (!window.confirm(`Are you sure you want to reject all ${payments.length} pending payments?`)) {
      return;
    }
    ...
    try {
      ...
      await Promise.all(
        payments.map(payment => 
          axios.post(`/api/section-payments/${payment._id}/reject`, {
      ...
      toast.success('All payments rejected successfully');
      fetchPendingPayments();
    } catch (error) {
      console.error('Error rejecting all payments:', error);
      toast.error('Failed to reject all payments');
    }
  };

  const viewReceipt = (receiptUrl) => {
    // Use axios base URL or fallback to localhost
    if (receiptUrl.startsWith('http')) {
      window.open(receiptUrl, '_blank');
    } else {
      const backendUrl = `${axios.defaults.baseURL || 'http://localhost:5000'}${receiptUrl}`;
  ...
  const downloadReceipt = async (receiptUrl, studentName) => {
    try {
      const fullUrl = receiptUrl.startsWith('http') ? receiptUrl : `${axios.defaults.baseURL || 'http://localhost:5000'}${receiptUrl}`;
      const token = localStorage.getItem('token');
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600 dark:text-gray-400">
                Verify student payments for course sections
              </p>
            </div>
            {payments.length > 0 && (
        ...
        {payments.length === 0 ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {payments.map((payment) => payment && payment._id ? (
                <div key={`payment-${payment._id}`} className="p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-4 flex items-center space-x-6 text-sm">
                        <div>
                          <p className="text-gray-500 dark:text-gray-400">Original Price</p>
                          <p className="font-medium text-gray-900 dark:text-white">
                            {formatPrice(payment.baseAmountSYP || payment.originalCoursePrice || payment.amountCents, payment.currency)}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500 dark:text-gray-400">Student Pays</p>
                          <p className="font-medium text-gray-900 dark:text-white">
                            {formatPrice(payment.finalAmountCents || payment.amountCents, payment.currency)}
                          </p>
                        </div>
                        <div>
                          <p className="text-gray-500 dark:text-gray-400">Wallet Used</p>
                          <p className="font-medium text-gray-900 dark:text-white">
                            {payment.balanceUsed > 0
                              ? `- ${formatPrice(payment.balanceUsed, payment.currency)}`
                              : 'No balance'}
                          </p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          {payment.receipt?.url && (
                            <div className="flex items-center space-x-2">
                              <img 
                                src={payment.receipt.url.startsWith('http') ? payment.receipt.url : `${axios.defaults.baseURL || 'http://localhost:5000'}${payment.receipt.url}`} 
                                alt="Receipt preview" 
