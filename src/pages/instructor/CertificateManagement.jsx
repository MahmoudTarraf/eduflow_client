...
const CertificateManagement = () => {
  const { user } = useAuth();
  const cannotIssueCertificates = isInstructorRestricted(user, 'issueCertificates');
  const [requests, setRequests] = useState([]);
  const [allRequests, setAllRequests] = useState([]);
  const [counts, setCounts] = useState({ requested: 0, issued: 0, rejected: 0 });
  ...
  const fetchAllRequests = async () => {
    try {
      ...
      const res = await axios.get('/api/certificates/requests', {
      ...
      const newCounts = {
        requested: allReqs.filter(r => r.status === 'requested').length,
        issued: allReqs.filter(r => r.status === 'issued').length,
        rejected: allReqs.filter(r => r.status === 'rejected').length
      };
      setCounts(newCounts);
      
      // Set initial filtered requests
      setRequests(allReqs.filter(r => r.status === activeTab));
    } catch (error) {
      console.error('Error fetching certificate requests:', error);
      toast.error(error.response?.data?.message || 'Failed to fetch certificate requests');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (requestId) => {
    if (cannotIssueCertificates) {
      toast.error('Your instructor account is suspended. You cannot approve certificate requests right now.');
      return;
    }
  ...
  const handleReject = async (requestId, reason) => {
    if (cannotIssueCertificates) {
      toast.error('Your instructor account is suspended. You cannot reject certificate requests right now.');
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {cannotIssueCertificates && (
          <div className="mb-6 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 text-yellow-800 dark:text-yellow-100 px-4 py-3 rounded">
            Your instructor account is currently suspended from issuing certificates. You can view requests, but approval and rejection actions are disabled.
          </div>
        )}
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500">
                          ...
                          {activeTab === 'issued' && request.issuedAt && (
                            <> • Issued on: {formatDate(request.issuedAt)}</>
                          )}
                      ...
                      {activeTab === 'requested' ? (
                        <>
                          <button
                            ...
                            disabled={cannotIssueCertificates}
                            className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 focus:outline-none"
                          >
                          ...
                          <button
                            ...
                            disabled={processing[request._id] || cannotIssueCertificates}
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          {request.certificateFile?.url && (
                            <button
                              onClick={() => {
                                const fullUrl = request.certificateFile.url.startsWith('http') 
                                  ? request.certificateFile.url 
                                  : `${axios.defaults.baseURL || 'http://localhost:5000'}${request.certificateFile.url}`;
                    ...
                    <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <button
                          ...
                          disabled={processing[request._id] || !certificateFile || cannotIssueCertificates}
